...
RATE_LIMIT_WINDOW = 5          # giây
RATE_LIMIT_MAX = 25            # số request tối đa trong 5 giây

def is_rate_limited(player_id: str) -> bool:
    now = time.time()
    timestamps = RATE_LIMIT[player_id]
    RATE_LIMIT[player_id] = [t for t in timestamps if now - t < RATE_LIMIT_WINDOW]
    
    if len(RATE_LIMIT[player_id]) >= RATE_LIMIT_MAX:
        return True
    RATE_LIMIT[player_id].append(now)
    return False
...
async def broadcast_to_room(room_id, payload_builder=None):
    ...
    for pid, p in list(room.players.items()) + list(room.spectators.items()):
        if p.connected and p.websocket and not p.websocket.closed:
            ...
            tasks.append(send_to_client(p.websocket, payload.get("event"), payload.get("data")))
...
async def nope_timer(room_id):
    try:
        ...
        if room_id in room_manager.rooms:
            ...
            if executed and original_action.get("card") == CardType.SEE_THE_FUTURE:
                player_ws = room.players.get(original_action["player_id"], {}).get("websocket")
                if player_ws:
...
async def handle_client(websocket, path):
    ...
    try:
        async for message in websocket:
            ...
            # Rate limiting
            if player_id and is_rate_limited(player_id):
                await send_to_client(websocket, "ERROR", None, "RATE_LIMIT_EXCEEDED")
                continue
            ...
            if event == "CREATE_ROOM":
                ...
                if not current_room_id or current_room_id not in room_manager.rooms:
                    continue
                room = room_manager.rooms[current_room_id]
